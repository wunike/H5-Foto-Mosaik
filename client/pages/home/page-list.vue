    ...
    <div class="clearfix my-page-list">
        ...
        <div class="page-content">
            <div class="my-page-nav-list">
                ...
                <div class="my-page-nav-item" @click="doSearch('share')" :class="{ active: searchParams.type === 'share' }">参与作品({{ shareCount }})</div>
            </div>
            ...
            <ul class="page-item-wrapper">
                ...
                <li class="page-item" v-for="(item, index) in pageList" :key="index">
                    ...
                    <div class="header-mask"><div class="details-btn" @click="showPreviewFn(item._id)">预览</div></div>
                    <div class="cover"><img :src="item.coverImage || defaultCoverImage" alt="" /></div>
                    <div class="page-item-title border-T">
                        <span class="item-title-i">{{ item.title }}</span>
                    </div>
                    <div class="page-item-data-pv border-T">
                        <div class="btn-wrapper"><el-button type="text" size="mini" @click="editPage(item._id)">编辑</el-button></div>
                        <div class="btn-wrapper"><el-button type="text" size="mini" @click="copyPage(item._id)">复制</el-button></div>
                        <div class="btn-wrapper">
                            <el-dropdown placement="top-start">
                                ...
                                <el-dropdown-menu slot="dropdown">
                                    <el-dropdown-item v-if="!item.isPublish && searchParams.type === 'my'"><div @click="publishPage(item._id, index)">发布</div></el-dropdown-item>
                                    <el-dropdown-item><div @click="setAsTemplate(item._id)">复制为模板</div></el-dropdown-item>
                                    <el-dropdown-item><div @click="showPageData(item._id)">页面数据</div></el-dropdown-item>
                                    <el-dropdown-item v-if="searchParams.type === 'my'"><div @click="showAddUser(item._id)">添加成员</div></el-dropdown-item>
                                    <el-dropdown-item v-if="searchParams.type === 'my'"><div @click="deletePage(item._id, index)">删除</div></el-dropdown-item>
                                    <el-dropdown-item v-if="searchParams.type === 'share'"><div @click="deleteShareUserPage(item._id, index)">移出我的参与</div></el-dropdown-item>
                                </el-dropdown-menu>
                            </el-dropdown>
                        </div>
                    </div>
            ...
            <el-dialog title="添加成员" :visible.sync="dialogVisible" width="500px">
                <el-form class="" label-width="120px">
                    <el-form-item label="用户名/姓名:">
                        ...
                        >
                            <el-option v-for="item in userDataList" :key="item._id" :label="item.username" :value="item._id"></el-option>
                        </el-select>
                    </el-form-item>
                </el-form>
...
export default {
    ...
    methods: {
        ...
        createNewPage() {
            ...
            this.loading = true;
            this.$axios
                .post('/page/add', { ...newPageData, pageMode: this.searchParams.pageMode, author: this.$store.state.user.userId })
                .then(res => {
                    this.loading = false;
                    if (res.body) {
        ...
        submitAddUser() {
            this.loading = true;
            this.$axios
                .post('/page/shareToUser/' + this.addUserForm.id, { userIds: this.addUserForm.userIds })
                .then(() => {
                    this.loading = false;
